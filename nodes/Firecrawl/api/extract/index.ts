...
// The v2 `/extract` endpoint is deprecated: API responses carry a deprecation warning pointing
// to `/scrape` with a JSON format, and the docs recommend `/agent` for discovery use cases.
const EXTRACT_DEPRECATED_NOTICE_TEXT =
	'The Extract API is deprecated. For a single page, use Scraping → Scrape URL and add JSON under Formats in Scrape Options. For data from multiple pages, use Agent → Extract Data With AI Agent. <a href="https://docs.firecrawl.dev/developer-guides/usage-guides/choosing-the-data-extractor" target="_blank">Learn more</a>';
